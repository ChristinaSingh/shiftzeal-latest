...
class ShiftDetailsController extends ChangeNotifier {
  bool inAsyncCall = false;
  ShiftDetailsById? shiftDetailsById;
  void initMethod(BuildContext context, String shiftId) {
    getMyShiftDetail(context: context, shiftId: shiftId);
  }

  Future<void> getMyShiftDetail(
      {required BuildContext context, required String shiftId}) async {
    inAsyncCall = true;
    notifyListeners();
    ...
    try {
      ...
      final GetMyShiftDetailsModel? getMyShiftModel =
          await ApiMethods.getMyShiftsDetailsApi(
        context: context,
      ...
      if (getMyShiftModel != null &&
        ...
        shiftDetailsById = getMyShiftModel.data;
        notifyListeners();
  ...
  clickOnTaskList({required BuildContext context}) {
    NavigationMethods.pushMethod(
      context: context,
      widget: TaskScreen(
        taskId: shiftDetailsById?.id.toString() ?? "",
      ),
    );
  }
}
